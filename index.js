...
const config = {
    ...
    instagram: 'https://instagram.com/shopgame_.yass', // INFO JANGAN DI UBAH
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Selamat tinggal😒 @${num.split('@')[0]} JANGAN BALIK LAGI ASW🐣😌`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
break
       ...
       client.sendMessage(from, 'Tuh udah aku kasih nomor Owner ajg⚜️Yass Bot⚜️,Jngn lupa di save su  ><',MessageType.text, { quoted: mek} )
           break    
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions('ENAK OM DI KICK? @${mentioned[0].split('@')[0]} awokawokawok🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂?? ??𝗶𝗺𝗶 𝗱𝗶 𝗴𝗿??𝘂𝗽 ??𝗻𝗶️')
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('𝙡𝙪 Sehat Tod?') 
